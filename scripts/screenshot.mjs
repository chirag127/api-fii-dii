...
// Invoke via a shell so `npx` resolves correctly on both Windows and POSIX.
execSync(
  `npx --yes playwright screenshot --full-page --viewport-size=1000,900 --wait-for-timeout=3000 "${url}" "${out}"`,
  { stdio: 'inherit' }
);
